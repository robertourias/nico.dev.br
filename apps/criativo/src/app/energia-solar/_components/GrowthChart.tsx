...
const PAD_TOP = 20
const PAD_BOTTOM = 32
const FILL_GRADIENT_ID = "solar-growth-fill"
const CLIP_PATH_ID = "solar-growth-clip"

// Gráfico de crescimento (economia acumulada mês a mês) em SVG puro — sem
// libs de chart, para manter o bundle leve. A entrada é animada revelando
// progressivamente a área via `clipPath` cuja largura cresce com
// `requestAnimationFrame`, disparado por IntersectionObserver. Reanima de
// forma mais rápida quando a série muda (recálculo do simulador).
    ...
    >
      ...
      <defs>
        <linearGradient id={FILL_GRADIENT_ID} x1="0%" y1="0%" x2="0%" y2="100%">
          <stop offset="0%" stopColor="#1C7FA6" stopOpacity={0.32} />
      ...
      <g clipPath={`url(#${CLIP_PATH_ID})`}>
        ...
        <path d={linePath} fill="none" stroke="var(--solar-blue-light)" strokeWidth={2.5} strokeLinejoin="round" strokeLinecap="round" />
        {points.map(([x, y], i) => (
          <circle
            key={i}
            cx={x}
            ...
            fill={i === points.length - 1 ? "var(--solar-yellow-deep)" : "var(--solar-blue-light)"}
            stroke="#fff"
            strokeWidth={1.5}
          />
        ))}
