      ...
      <div className="max-w-3xl mx-auto">
        <Reveal className="text-center mb-14">
          <h2 id="testimonials-title" className="text-3xl md:text-4xl font-bold mb-4 text-foreground">
            O que nossos <span className="text-[var(--color-primary)]">pacientes dizem</span>
          </h2>
        </Reveal>
        ...
        >
          <div className="clin-carousel-track" style={{ transform: `translateX(-${index * 100}%)` }}>
            {TESTIMONIALS.map((t) => (
              <figure key={t.name} className="w-full shrink-0 px-1">
                <div className="clin-glass clin-glow rounded-3xl p-8 md:p-10 text-center">
                  ...
                  <div className="flex items-center justify-center gap-0.5 mb-4" aria-hidden="true">
                    {Array.from({ length: 5 }, (_, i) => (
                      <Star
                        key={i}
                        className={`size-4 ${i < t.rating ? "fill-[var(--clin-green)] text-[var(--clin-green)]" : "text-border"}`}
                      />
                    ))}
                  </div>
          ...
          <button
            ...
            className="hidden sm:flex absolute left-0 top-1/2 -translate-y-1/2 -translate-x-3 items-center justify-center size-10 rounded-full clin-glass text-foreground hover:text-[var(--color-primary)] transition-colors"
          >
          ...
          <button
            ...
            className="hidden sm:flex absolute right-0 top-1/2 -translate-y-1/2 translate-x-3 items-center justify-center size-10 rounded-full clin-glass text-foreground hover:text-[var(--color-primary)] transition-colors"
          >
