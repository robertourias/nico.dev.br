...
// e alterna `data-visible`, que o CSS em theme.css (`.nexus-reveal`) anima.
// IntersectionObserver puro (sem libs de animação), mesmo padrão usado em
// `pre-venda-metodo-apice`. Dispara uma única vez.
export function Reveal({ children, variant = "fade-up", delayMs = 0, className, ...rest }: RevealProps) {
  ...
  return (
    <div
      ref={ref}
      className={`nexus-reveal ${className ?? ""}`}
