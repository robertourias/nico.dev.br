...
export function FaqSection() {
  ...
  return (
    <section id="faq" className="px-6 py-20 md:py-28 scroll-mt-20 bg-[var(--color-surface)]" aria-labelledby="faq-title">
      <div className="max-w-2xl mx-auto">
        <Reveal className="text-center mb-10">
          <h2 id="faq-title" className="text-3xl md:text-4xl font-bold text-foreground">
            Perguntas <span className="aca-highlight">Frequentes</span>
          </h2>
        </Reveal>
          ...
          {FAQS.map((faq, index) => {
            ...
            return (
              <div key={faq.question} className="rounded-2xl border border-border bg-[var(--color-surface-raised)] overflow-hidden">
                ...
                {isOpen && (
                  <div className="aca-accordion-content px-5 pb-4 text-sm text-muted-foreground leading-relaxed">
                    {faq.answer}
                  </div>
                )}
              </div>
